"""Optional ``linear_logp`` provider boundary.

Vime owns token-row layout and loss composition. An installed provider owns
the numerical implementation, distributed reduction, and backend provenance.
The boundary is intentionally model- and provider-package agnostic.
"""

from __future__ import annotations

import importlib
import logging
from collections.abc import Callable, Mapping
from dataclasses import dataclass, field
from typing import Any, Literal, Protocol, runtime_checkable

import torch

logger = logging.getLogger(__name__)
_LOGGED_PROVIDER_IDENTITIES: set[tuple[str, str]] = set()
_LOGGED_NATIVE_IDENTITIES: set[tuple[str, str, str]] = set()
_NATIVE_CONTRACT_ID = "vime.native.linear_logp.v1"

ProviderMode = Literal["auto", "strict"]
TokenLayoutKind = Literal["single", "zigzag", "allgather"]


class LinearLogpProviderUnavailable(RuntimeError):
    """Request Vime's native implementation in ``auto`` mode."""

    linear_logp_provider_unavailable = True


@dataclass(frozen=True)
class TokenLayout:
    """Ownership of local token rows handed to a provider."""

    world_size: int
    rank: int
    layout: TokenLayoutKind

    def __post_init__(self) -> None:
        if self.world_size < 1 or not 0 <= self.rank < self.world_size:
            raise ValueError(f"invalid token topology: world_size={self.world_size}, rank={self.rank}")
        if self.world_size == 1 and self.layout != "single":
            raise ValueError("single-rank token parallelism must use layout='single'")
        if self.world_size > 1 and self.layout == "single":
            raise ValueError("multi-rank token parallelism must use zigzag or allgather layout")


@dataclass(frozen=True)
class LinearProjection:
    """Local projection used by ``linear_logp``."""

    weight: torch.Tensor
    bias: torch.Tensor | None = None

    def __post_init__(self) -> None:
        if not isinstance(self.weight, torch.Tensor) or self.weight.ndim != 2:
            raise ValueError("linear_logp projection.weight must be a rank-2 tensor")
        if self.bias is not None:
            if not isinstance(self.bias, torch.Tensor):
                raise ValueError("linear_logp projection.bias must be a tensor")
            if self.bias.ndim != 1 or self.bias.size(0) != self.weight.size(0):
                raise ValueError("linear_logp projection.bias must match local vocabulary width")
            if self.bias.device != self.weight.device:
                raise ValueError("linear_logp projection tensors must share a device")


@dataclass(frozen=True)
class VocabPartition:
    """Contiguous local vocabulary shard in the global padded vocabulary."""

    local_start: int
    local_size: int
    real_size: int
    padded_size: int

    def __post_init__(self) -> None:
        if self.local_start < 0 or self.local_size <= 0:
            raise ValueError("linear_logp vocabulary partition must have a non-empty non-negative local range")
        if self.real_size <= 0 or self.padded_size <= 0 or self.real_size > self.padded_size:
            raise ValueError("linear_logp vocabulary sizes are invalid")
        if self.local_start + self.local_size > self.padded_size:
            raise ValueError("linear_logp local vocabulary range exceeds padded vocabulary")


@dataclass(frozen=True)
class LinearLogpContext:
    """Structural inputs that let a provider avoid a materialized LM-head output."""

    hidden: torch.Tensor
    projection: LinearProjection
    vocab_partition: VocabPartition

    def __post_init__(self) -> None:
        if not isinstance(self.hidden, torch.Tensor) or self.hidden.ndim != 2:
            raise ValueError("linear_logp context.hidden must be [T, hidden]")
        if self.hidden.size(1) != self.projection.weight.size(1):
            raise ValueError("linear_logp hidden width must match projection width")
        if self.projection.weight.size(0) != self.vocab_partition.local_size:
            raise ValueError("linear_logp projection width must match vocabulary partition")
        if self.hidden.device != self.projection.weight.device:
            raise ValueError("linear_logp context tensors must share a device")


@dataclass(frozen=True)
class LinearLogpRequest:
    """Normalized inputs supplied by Vime's Megatron backend.

    ``logits`` is retained for the native path and for provider-side metrics
    such as full-vocabulary entropy. ``context`` carries the model-neutral
    structural contract used by fused providers.
    """

    logits: torch.Tensor
    target_ids: torch.Tensor
    tensor_parallel_group: Any
    token_layout: TokenLayout
    with_entropy: bool
    with_entropy_grad: bool
    chunk_size: int
    log_prob_keep_mask: torch.Tensor | None = None
    context: LinearLogpContext | None = None
    temperature: float | torch.Tensor | None = None
    metadata: Mapping[str, Any] = field(default_factory=dict)

    def __post_init__(self) -> None:
        if not isinstance(self.logits, torch.Tensor) or self.logits.ndim != 2:
            raise ValueError(f"linear_logp logits must be [T, V_local], got {getattr(self.logits, 'shape', None)}")
        if self.target_ids.shape != (self.logits.size(0),):
            raise ValueError(
                "linear_logp target_ids must be [T] aligned with logits; "
                f"got {tuple(self.target_ids.shape)} for T={self.logits.size(0)}"
            )
        if self.target_ids.device != self.logits.device:
            raise ValueError("linear_logp target_ids must share the logits device")
        if self.target_ids.is_floating_point() or self.target_ids.is_complex():
            raise TypeError("linear_logp target_ids must use an integer dtype")
        if self.log_prob_keep_mask is not None and self.log_prob_keep_mask.shape != self.logits.shape:
            raise ValueError("linear_logp log_prob_keep_mask must match logits shape")
        if self.context is not None:
            if self.context.hidden.size(0) != self.logits.size(0):
                raise ValueError("linear_logp context rows must match local logits rows")
            if self.context.projection.weight.size(0) != self.logits.size(1):
                raise ValueError("linear_logp projection rows must match local logits width")
            if self.context.hidden.device != self.logits.device:
                raise ValueError("linear_logp context must share the logits device")
        if self.temperature is not None:
            if isinstance(self.temperature, torch.Tensor):
                if self.temperature.numel() not in (1, self.logits.size(0)):
                    raise ValueError("linear_logp temperature must be scalar or [T]")
                if bool((self.temperature <= 0).any().item()):
                    raise ValueError("linear_logp temperature must be positive")
            elif float(self.temperature) <= 0:
                raise ValueError("linear_logp temperature must be positive")


@dataclass(frozen=True)
class LinearLogpResult:
    """Provider result and immutable identity used for runtime readback."""

    logp: torch.Tensor
    entropy: torch.Tensor | None
    backend_id: str
    contract_id: str
    provenance: Mapping[str, Any] = field(default_factory=dict)


@runtime_checkable
class LinearLogpProvider(Protocol):
    def __call__(self, request: LinearLogpRequest) -> LinearLogpResult: ...


def linear_logp_provider_path(args: Any) -> str | None:
    path = getattr(args, "linear_logp_provider", None)
    if path is None:
        return None
    normalized = str(path).strip()
    return normalized or None


def linear_logp_provider_mode(args: Any) -> ProviderMode:
    mode = str(getattr(args, "linear_logp_provider_mode", "auto")).strip().lower()
    if mode not in {"auto", "strict"}:
        raise ValueError(
            "linear_logp_provider_mode must be 'auto' or 'strict', "
            f"got {getattr(args, 'linear_logp_provider_mode', None)!r}."
        )
    return mode  # type: ignore[return-value]


def compute_linear_logp(
    *,
    args: Any,
    request: LinearLogpRequest,
    native: Callable[..., tuple[torch.Tensor, torch.Tensor | None]],
) -> tuple[torch.Tensor, torch.Tensor | None]:
    """Use an external provider when configured, otherwise execute native code.

    ``auto`` falls back only when the provider cannot be loaded or explicitly
    raises :class:`LinearLogpProviderUnavailable`. Provider bugs and contract
    violations are never hidden by a native fallback.
    """

    path = linear_logp_provider_path(args)
    if path is None:
        return _native(request, native, route="unconfigured")

    mode = linear_logp_provider_mode(args)
    try:
        provider = _load_provider(path)
    except (ImportError, AttributeError) as exc:
        if mode == "strict":
            raise RuntimeError(f"linear_logp provider {path!r} is unavailable: {exc}") from exc
        logger.warning("linear_logp provider %s is unavailable; using native path: %s", path, exc)
        return _native(request, native, route="provider_unavailable_fallback")
    try:
        result = provider(request)
    except Exception as exc:
        if not _is_provider_unavailable(exc):
            raise
        if mode == "strict":
            raise RuntimeError(f"linear_logp provider {path!r} is unavailable: {exc}") from exc
        logger.warning("linear_logp provider %s is unavailable; using native path: %s", path, exc)
        return _native(request, native, route="provider_unavailable_fallback")

    normalized = _normalize_result(result)
    _validate_result(normalized, request, strict=mode == "strict")
    _log_provider_identity(normalized)
    return normalized.logp, normalized.entropy


def _load_provider(path: str) -> LinearLogpProvider:
    module_path, separator, attribute = path.rpartition(".")
    if not separator or not module_path or not attribute:
        raise ImportError("provider must use the import path 'package.module.callable'")
    provider = getattr(importlib.import_module(module_path), attribute)
    if not callable(provider):
        raise TypeError(f"linear_logp provider {path!r} is not callable")
    return provider


def _native(
    request: LinearLogpRequest,
    native: Callable[..., tuple[torch.Tensor, torch.Tensor | None]],
    *,
    route: str,
) -> tuple[torch.Tensor, torch.Tensor | None]:
    result = native(
        request.logits,
        request.target_ids,
        request.tensor_parallel_group,
        with_entropy=request.with_entropy,
        with_entropy_grad=request.with_entropy_grad,
        chunk_size=request.chunk_size,
        log_prob_keep_mask=request.log_prob_keep_mask,
    )
    _log_native_identity(native, route=route, device_type=request.logits.device.type)
    return result


def _log_native_identity(
    native: Callable[..., Any], *, route: str, device_type: str
) -> None:
    module = getattr(native, "__module__", type(native).__module__)
    qualname = getattr(native, "__qualname__", type(native).__qualname__)
    backend_id = f"{module}.{qualname}"
    identity = (backend_id, route, device_type)
    if identity in _LOGGED_NATIVE_IDENTITIES:
        return
    _LOGGED_NATIVE_IDENTITIES.add(identity)
    logger.info(
        "linear_logp native active: backend_id=%s contract_id=%s route=%s device=%s",
        backend_id,
        _NATIVE_CONTRACT_ID,
        route,
        device_type,
    )


def _is_provider_unavailable(exc: Exception) -> bool:
    """Keep provider packages independent from Vime's exception class."""

    return bool(getattr(exc, "linear_logp_provider_unavailable", False))


def _normalize_result(result: Any) -> LinearLogpResult:
    """Accept Vime's dataclass or a structural result from an external package."""

    if isinstance(result, LinearLogpResult):
        return result
    if isinstance(result, Mapping):

        def get(name):
            return result[name]

    else:

        def get(name):
            return getattr(result, name)

    try:
        provenance = get("provenance")
        logp = get("logp")
        entropy = get("entropy")
        backend_id = get("backend_id")
        contract_id = get("contract_id")
    except (AttributeError, KeyError) as exc:
        raise TypeError(
            "linear_logp providers must return logp, entropy, backend_id, contract_id, and provenance"
        ) from exc
    if not isinstance(provenance, Mapping):
        raise TypeError("linear_logp provider provenance must be a mapping")
    return LinearLogpResult(
        logp=logp,
        entropy=entropy,
        backend_id=backend_id,
        contract_id=contract_id,
        provenance=provenance,
    )


def _validate_result(result: LinearLogpResult, request: LinearLogpRequest, *, strict: bool) -> None:
    if result.logp.shape != (request.logits.size(0), 1):
        raise ValueError(
            "linear_logp provider returned invalid logp shape "
            f"{tuple(result.logp.shape)}; expected {(request.logits.size(0), 1)}."
        )
    if result.logp.device != request.logits.device:
        raise ValueError("linear_logp provider returned logp on a different device")
    if not result.logp.is_floating_point():
        raise TypeError("linear_logp provider must return floating-point logp")
    if request.with_entropy:
        if result.entropy is None or result.entropy.shape != (request.logits.size(0),):
            raise ValueError("linear_logp provider must return [T] entropy when with_entropy=True")
        if result.entropy.device != request.logits.device:
            raise ValueError("linear_logp provider returned entropy on a different device")
        if not result.entropy.is_floating_point():
            raise TypeError("linear_logp provider must return floating-point entropy")
    elif result.entropy is not None:
        raise ValueError("linear_logp provider returned entropy when with_entropy=False")
    if strict:
        if not result.backend_id or not result.contract_id:
            raise ValueError("strict linear_logp runs require non-empty backend_id and contract_id")
        if request.logits.requires_grad and not result.logp.requires_grad:
            raise ValueError("strict linear_logp result is detached from autograd")
        if request.with_entropy_grad and result.entropy is not None and not result.entropy.requires_grad:
            raise ValueError("strict linear_logp entropy is detached from autograd")


def _log_provider_identity(result: LinearLogpResult) -> None:
    identity = (result.backend_id, result.contract_id)
    if identity in _LOGGED_PROVIDER_IDENTITIES:
        return
    _LOGGED_PROVIDER_IDENTITIES.add(identity)
    logger.info(
        "linear_logp provider active: backend_id=%s contract_id=%s provenance=%s",
        result.backend_id,
        result.contract_id,
        dict(result.provenance),
    )


__all__ = [
    "LinearLogpContext",
    "LinearLogpProvider",
    "LinearLogpProviderUnavailable",
    "LinearLogpRequest",
    "LinearLogpResult",
    "LinearProjection",
    "TokenLayout",
    "VocabPartition",
    "compute_linear_logp",
    "linear_logp_provider_mode",
    "linear_logp_provider_path",
]
