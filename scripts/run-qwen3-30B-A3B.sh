...
export TMPDIR="${TMPDIR:-${WORKSPACE_ROOT}/.cache/tmp}"
export FLASHINFER_WORKSPACE_BASE="${FLASHINFER_WORKSPACE_BASE:-${WORKSPACE_ROOT}}"
export FLASHINFER_CUDA_ARCH_LIST="${FLASHINFER_CUDA_ARCH_LIST:-9.0}"
export VIME_SKIP_MOE_GROUPED_GEMM_CAPABILITY_CHECK="${VIME_SKIP_MOE_GROUPED_GEMM_CAPABILITY_CHECK:-1}"
mkdir -p "${HF_HOME}" "${HUGGINGFACE_HUB_CACHE}" "${TRANSFORMERS_CACHE}" \
   "${PIP_CACHE_DIR}" "${TORCH_HOME}" "${XDG_CACHE_HOME}" "${TMPDIR}" \
...
VIME_UPDATE_WEIGHT_BUFFER_SIZE=${VIME_UPDATE_WEIGHT_BUFFER_SIZE:-}
...
echo "VLLM_PROFILER_CONFIG: ${VLLM_PROFILER_CONFIG:-<unset>}"
...
echo "VIME_UPDATE_WEIGHT_BUFFER_SIZE: ${VIME_UPDATE_WEIGHT_BUFFER_SIZE:-<unset>}"
...
if [[ -n "${VLLM_PROFILER_CONFIG}" ]]; then
   VLLM_ARGS+=(--vllm-profiler-config "${VLLM_PROFILER_CONFIG}")
fi
...
if [[ -n "${VIME_UPDATE_WEIGHT_BUFFER_SIZE}" ]]; then
   MISC_ARGS+=(--update-weight-buffer-size "${VIME_UPDATE_WEIGHT_BUFFER_SIZE}")
fi
...
cd "${VIME_ROOT}"
if [[ "${VIME_SKIP_RAY_START:-0}" != "1" ]]; then
...
RUNTIME_ENV_JSON="{
  \"env_vars\": {
    ...
    \"VIME_SKIP_MOE_GROUPED_GEMM_CAPABILITY_CHECK\": \"${VIME_SKIP_MOE_GROUPED_GEMM_CAPABILITY_CHECK}\",
    ...
    \"VIME_BASELINE_OUTPUT_LAYER_DETACH_HIDDEN\": \"${VIME_BASELINE_OUTPUT_LAYER_DETACH_HIDDEN:-}\",
    ...
    \"RL_KERNEL_LINEAR_LOGP_SAVE_PROBS_ROWWISE_DLOGITS\": \"${RL_KERNEL_LINEAR_LOGP_SAVE_PROBS_ROWWISE_DLOGITS:-0}\",
    \"RL_KERNEL_LINEAR_LOGP_FUSED_BWD_ROWWISE_DLOGITS\": \"${RL_KERNEL_LINEAR_LOGP_FUSED_BWD_ROWWISE_DLOGITS:-0}\",
    \"RL_KERNEL_LINEAR_LOGP_FUSED_TILE_BWD_FULL\": \"${RL_KERNEL_LINEAR_LOGP_FUSED_TILE_BWD_FULL:-0}\",
    \"RL_KERNEL_LINEAR_LOGP_STREAMING_BWD_VOCAB_TILE\": \"${RL_KERNEL_LINEAR_LOGP_STREAMING_BWD_VOCAB_TILE:-}\",
    ...
    \"VIME_VLLM_FAULTHANDLER\": \"${VIME_VLLM_FAULTHANDLER:-0}\",
    \"VLLM_WORKER_MULTIPROC_METHOD\": \"${VLLM_WORKER_MULTIPROC_METHOD}\",
    \"VIME_SKIP_ZERO_ENTROPY_METRIC\": \"${VIME_SKIP_ZERO_ENTROPY_METRIC:-0}\",
    \"VIME_NSYS_CAPTURE_ROLLOUTS\": \"${VIME_NSYS_CAPTURE_ROLLOUTS:-}\",
    \"VIME_NSYS_CAPTURE_ROLE\": \"${VIME_NSYS_CAPTURE_ROLE:-actor}\",
